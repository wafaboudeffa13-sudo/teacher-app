...
function resizeCanvas() {
  ...
  fitScale = Math.min(w.clientWidth/LOGICAL_W, w.clientHeight/LOGICAL_H);
...
async function toggleMic() {
  ...
  if (!micOn) {
    try {
      ...
      recordTeacherClip();
    } catch(err) { console.error(err); showToast('❌ تعذر فتح المايك (HTTPS؟)'); }
  } else {
...
async function toggleStudentMic() {
  ...
  if (!studentMicOn) {
    try {
      ...
      showToast('🎙️ مايكك شاغل!');
      recordStudentClip();
    } catch(err) { showToast('❌ تعذر فتح المايك'); }
  } else {
...
function recordStudentClip() {
  ...
  const mime=getMime(); const chunks=[];
  let rec;
  try { rec=new MediaRecorder(studentStream, mime?{mimeType:mime,audioBitsPerSecond:64000}:{}); } catch(e) { return; }
  studentRecorder=rec;
  rec.ondataavailable=e => { if (e.data?.size>0) chunks.push(e.data); };
  rec.onstop=async () => {
    if (chunks.length>0) {
...
function playAudio(sourceId, data) {
  ...
  const q=playQueues.get(sourceId); q.items.push(data);
...
function playNext(sourceId) {
  ...
  const audio=new Audio(url); audio.playsInline=true;
...
function removeMsgEl(id) { const el=chatMessagesEl.querySelector(`[data-id="${id}"]`); if (el) el.remove(); }
...
function addPending(d) {
  ...
  if (document.getElementById('pending_'+d.id)) return;
...
function setupSocketEvents() {
  ...
  socket.on('approve_failed', () => showToast('⚠️ التلميذ ما عادش — اطلبو يعاود'));
  socket.on('init', s => {
    ...
    if (ROLE==='teacher') {
      updateRaisedHands(s.raisedHands||[]);
      (s.pendingStudents||[]).forEach(p=>addPending(p));
    }
  ...
  socket.on('pending_removed', d => removePending(d.id));
  ...
  socket.on('mic_revoked', () => {
    micGranted=false;
    studentMicBtn.classList.remove('granted'); studentMicBtn.classList.remove('on');
